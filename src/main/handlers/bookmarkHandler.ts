...
export function registerBookmarksHandlers(): void {
  ...
  ipcMain.handle('bookmark:get', async (_event, stage: string): Promise<any> => {
    try {
      ...
      // 해당 스테이지 폴더 찾기
      const targetFolder = bookmarkBar.children.find(
        (child: any) => child.type === 'folder' && child.name === stage
      );
   ...
   * @param url 북마크 URL
   */
  ipcMain.handle('bookmark:add', async (_event, stage: string, name: string, url: string): Promise<{ success: boolean; error?: string }> => {
    try {
      if (!STAGES.includes(stage)) {
